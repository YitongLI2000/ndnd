...
const BestRouteSuppressionTime = 20 * time.Millisecond
...
func (s *BestRoute) forwardInterestStandard(
	...
	// now := time.Now() // used by suppression logic; disabled for now
	for pass := range 2 {
		for _, nh := range nexthops {
			// In the first pass, skip hops that already have a out record
			if pass == 0 {
				if oR := pitEntry.OutRecords()[nh.Nexthop]; oR != nil {
					// Suppression logic is intentionally disabled for experiments.
					// Keep the original code here for easy restore.
