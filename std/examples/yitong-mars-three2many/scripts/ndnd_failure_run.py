...
# --- Helper for Failure Simulation ---
def get_face_id_for_neighbor(node, neighbor_ip):
    """
    Parses ndnd fw face-list to find the Face ID for a specific neighbor IP.
    Includes robust checking for the IP string within the remote URI.
    """
    output = node.cmd('ndnd fw face-list')
    # Debug print to see what the node sees
    # print(f"DEBUG: Face list for {node.name}:\n{output}")
    
    for line in output.split('\n'):
        # Check if the neighbor IP is present in the remote URI
        # The format is usually remote=udp4://IP:PORT
        if f"remote=udp4://{neighbor_ip}" in line:
            match = re.search(r'faceid=(\d+)', line)
            if match:
                return int(match.group(1))
    return None

def simulate_failure_recovery(net, allocator):
    print("\n=== Starting Failure and Recovery Simulation Sequence ===")
    
    # Define the scenarios: (Consumer Name, Target Core Name, Prefix Start Index, Prefix End Index)
    scenarios = [
        ('con0', 'core1', 0, 4),   # Remove routes to core1 for /pro0-/pro4
        ('con1', 'core2', 5, 9),   # Remove routes to core2 for /pro5-/pro9
        ('con2', 'core3', 10, 14)  # Remove routes to core3 for /pro10-/pro14
    ]

    # Store data needed for recovery later: (node_obj, face_id, p_start, p_end)
    recovery_data = []

    # 1. Wait 2 seconds after consumers started
    print("⏳ Waiting 2s before triggering link failure...")
    time.sleep(2)

    print("⚡ Triggering Link Failures (Route Removal)...")
    for con_name, core_name, p_start, p_end in scenarios:
        con_node = net.get(con_name)
        
        # Find the IP of the core router facing this consumer
        # Note: allocator.get_link_ip(core_name, con_name) returns the IP of core_name 
        # on the link connected to con_name.
        core_ip = allocator.get_link_ip(core_name, con_name)
        
        if not core_ip:
            print(f"  [Error] Could not find IP for {core_name} facing {con_name}")
            continue
            
        # Find the Face ID associated with that IP
        face_id = get_face_id_for_neighbor(con_node, core_ip)
        
        if face_id:
            print(f"  -> {con_name}: Removing routes to {core_name} (IP: {core_ip}, Face {face_id}) for /pro{p_start}-/pro{p_end}")
            for i in range(p_start, p_end + 1):
                prefix = f"/pro{i}app" # Note: Appended 'app' to match prefix in logs
                # Explicitly specifying origin=255 (static) to match the routes added via route-add
                # and to ensure we target the correct route entry.
                con_node.cmd(f"ndnd fw route-remove prefix={prefix} face={face_id} origin=255")
            
            # Save info for recovery
            recovery_data.append((con_node, face_id, p_start, p_end, con_name, core_name))
        else:
            print(f"  [Error] Face ID not found for {con_name} -> {core_name} ({core_ip})")

    # 2. Wait another 2 seconds (Total 4s from start)
    print("⏳ Waiting 2s (Total 4s from start) before recovery...")
    time.sleep(2)

    print("🚑 Triggering Route Recovery (Route Add)...")
    for con_node, face_id, p_start, p_end, c_name, core_name in recovery_data:
        print(f"  -> {c_name}: Restoring routes to {core_name} (Face {face_id})")
        for i in range(p_start, p_end + 1):
            prefix = f"/pro{i}app"
            # Adding back with cost=4 and origin=255 to match the state before removal
            # (Based on logs: origin=static cost=4)
            con_node.cmd(f"ndnd fw route-add prefix={prefix} face={face_id} cost=4 origin=255")

    print("=== Failure and Recovery Sequence Complete ===")

def run_applications(net, allocator):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    ...
    # Step 5: Execute Failure and Recovery Logic
    simulate_failure_recovery(net, allocator)
...
if __name__ == '__main__':
    ...
    try:
        ...
        run_applications(net, allocator)
        CLI(net)
